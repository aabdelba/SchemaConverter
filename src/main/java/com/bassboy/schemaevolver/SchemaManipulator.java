...
public class SchemaManipulator {
	...
	public static void oldSchema_unwrapRecordFromArray(BfsConditioner bfsConditioner, Entry<String, JsonNode> oldEntry, JsonNode oldParent) throws IOException, SchemaEvolverException {
		System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!! SCENARIO: record no longer wrapped in an array !!!!!!!!!!!!!!!!!!!!!!!!!!!!");

		//modify old schema
		ObjectMapper mapper = new ObjectMapper();
		oldEntry.setValue(mapper.readTree(oldEntry.getValue().get("items").toString()));
		System.out.println("   oldSchema node modified: " + oldEntry);

		//modify old json
		String parentName = oldParent.get("name").toString().replace("\"", "");
		oldJson_unwrapRecordFromArray(bfsConditioner,parentName);

	}

	private static void oldJson_unwrapRecordFromArray(BfsConditioner bfsConditioner, String segmentName) throws SchemaEvolverException, IOException {

		int i=0;
		for (JsonNode json : bfsConditioner.getJsonRecords()) {

			if(i==0)
				bfsConditioner.setJsonRecords(new ArrayList<>());
			...
			ObjectMapper mapper = new ObjectMapper();
			try {
				JsonNode innerJson = recordEntry.getValue();
				for (JsonNode arrayEntry : innerJson) {
					recordEntry.setValue(mapper.readTree(arrayEntry.toString()));
					String jsonStr = json.toString();
					bfsConditioner.addJsonRecord(jsonStr);
				}
	...
	public static void oldSchema_wrapRecordInArray(BfsConditioner bfsConditioner, Entry<String, JsonNode> oldEntry, JsonNode oldParent) throws IOException, SchemaEvolverException {
		...
		//modify old json
		String parentName = oldParent.get("name").toString().replace("\"", "");
		oldJson_wrapRecordInArray(bfsConditioner,parentName);

	}

	private static void oldJson_wrapRecordInArray(BfsConditioner bfsConditioner, String segmentName) throws IOException, SchemaEvolverException {
